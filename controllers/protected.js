  ...
  Organisation.findByIdAndUpdate(
    ...
    (err, organisation) => {
      ...
      const emailBody = {
        ...
        html: `<h3>Hello infoXchange</h3><p>A user from ${
          organisation.name
        } has updated the information for this organisation<br>The new information submitted is as follows....<br><br><ul>${dataUpdated.join(
          ""
        )}</ul></p>`
      };
...
router.post("/site/:org_id", (req, res) => {
  ...
  Organisation.findById(new ObjectId(org_id), (err, organisation) => {
    ...
    const emailBody = {
      ...
      html: `<h3>Hello infoXchange</h3><p>A user from ${
        organisation.name
      } has created a new site.<br>The data submitted for the new site is... <br><br><ul>${dataUpdated.join(
        ""
      )}</ul></p>`
    };
    transporter.sendMail(emailBody, function(error, info) {
...
router.put("/site/:org_id/:site_id", (req, res) => {
  ...
  Organisation.findById(new ObjectId(org_id), (err, organisation) => {
    ...
    const emailBody = {
      ...
      html: `<h3>Hello infoXchange</h3><p>A user from ${
        organisation.name
      } has updated details for the '${
        site.name
      }' Site<br>The data submitted for the new site is... <br><br><ul>${dataUpdated.join(
        ""
      )}</ul></p>`
    };
    transporter.sendMail(emailBody, function(error, info) {
...
router.delete("/site/:org_id/:site_id", (req, res) => {
  ...
  Organisation.findById(new ObjectId(org_id), (err, organisation) => {
    ...
    const emailBody = {
      ...
      html: `<h4>Hello infoeXchange</h4><p>A user from ${
        organisation.name
      } has deleted the '${site.name}' site</p>`
    };
    transporter.sendMail(emailBody, function(error, info) {
      if (error) {
        console.log(error);
...
router.put("/service/:org_id/:site_id/:service_id", (req, res) => {
  ...
  Organisation.findById(new ObjectId(org_id), (err, organisation) => {
    ...
    const emailBody = {
      ...
      html: `<h3>Hello infoXchange</h3><p>A user from ${
        organisation.name
      } has updated details for the '${service.name}' service under their '${
        site.name
      }' Site<br>The data submitted for the new site is... <br><br><ul>${dataUpdated.join(
        ""
      )}</ul></p>`
...
router.delete("/service/:org_id/:site_id/:service_id", (req, res) => {
  ...
  Organisation.findById(new ObjectId(org_id), (err, organisation) => {
    ...
    const emailBody = {
      ...
      html: `<h4>Hello infoeXchange</h4><p>A user from ${
        organisation.name
      } has deleted the service '${service.name}' which was provided at the '${
        site.name
      }' site`
    };
    transporter.sendMail(emailBody, function(error, info) {
